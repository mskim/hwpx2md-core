...
class Processor {
    ...
    convertBackslashToRoman(script) {
        return script.replace(/(^|\s)\\(?![a-zA-Z])/gm, (_m, prefix) => `${prefix}\\rm `);
    }
    decodeHtmlEntities(script) {
        return Object.entries(HTML_ENTITIES).reduce((s, [entity, char]) => s.replace(new RegExp(entity.replace(/[.*+?^${}()|[\]\\]/g, "\\$&"), "g"), char), script);
    }
    convertSubscriptSuperscript(script) {
