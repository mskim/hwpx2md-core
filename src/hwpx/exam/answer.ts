 ...
 */
function leadingValue(text: string): { digits: string; rest: string } {
  const eq = /^\$\$(\d+)\$\$/.exec(text);
  if (eq) return { digits: eq[1]!, rest: text.slice(eq[0].length) };
 ...
 */
export function splitAnswerAndExplanation(body: string, opts: SplitOptions): AnswerSplit {
  const text = body
    .trim()
  ...
  const tail = (s: string): string => s.replace(EXPLANATION_LABEL, "").trim();
